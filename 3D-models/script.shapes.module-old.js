...
// ──────────────────────────────────────────────────────────────────────────────
// Shapes registry (OBJ-baked meshes: vertices + faces)
// ──────────────────────────────────────────────────────────────────────────────
export const shapes = {};

/** Register a shape from raw arrays */
export function registerShape(name, vertices, faces) {
  if (!name) throw new Error('registerShape: name required');
  if (!Array.isArray(vertices) || vertices.length % 3 !== 0)
    throw new Error('registerShape: vertices must be [x,y,z,...]');
  if (!Array.isArray(faces) || faces.length % 3 !== 0)
    throw new Error('registerShape: faces must be [i0,i1,i2,...]');
  shapes[name] = { vertices, faces };
}

/** Register a shape by importing two tiny modules that export { figure } and { faces } */
export async function registerShapeFromModules(name, vertsUrl, facesUrl) {
  const [{ figure }, { faces }] = await Promise.all([import(vertsUrl), import(facesUrl)]);
  registerShape(name, figure, faces);
  return name;
}

/** Build a THREE.Mesh from a registered shape */
export function makeShape(name, materialOptions = { color: 0xE2AE68, flatShading: true }, opts = {}) {
  const def = shapes[name];
  if (!def) throw new Error(`Shape not found: ${name}`);

  const geo = new THREE.BufferGeometry();
  geo.setAttribute('position', new THREE.BufferAttribute(new Float32Array(def.vertices), 3));

  // Choose index type automatically (Uint16 vs Uint32)
  const maxIndex = def.faces.reduce((m, v) => (v > m ? v : m), 0);
  const IndexArray = (maxIndex > 65535) ? Uint32Array : Uint16Array;
  geo.setIndex(new THREE.BufferAttribute(new IndexArray(def.faces), 1));
  geo.computeVertexNormals();

  // Optional center/scale to a consistent visual size
  const { center = true, scaleTo = 50 } = opts;
  if (center || scaleTo) {
  ...
  mesh.userData.funebra = { shape: name };
  return mesh;
}
